...
(4) unet_segment_movie(images_path, checkpoint_path, numZ): loads desired model and segments every image contained in images_path. Handles each Z-plane as an individual 2D image,
	with no information shared between slices of the same volume (this is definitely an area that could be improved by a fully 3D approach!)
